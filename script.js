document.getElementById('uploadForm').addEventListener('submit', function (event) {
  ...
  // Get the media input (allowing for both images and videos)
  const mediaInput = document.getElementById('media'); // Change 'photo' to 'media' or whatever your input's ID is
  if (mediaInput.files.length > 0) {
      formData.append('media', mediaInput.files[0]); // Append the media file
  }
  ...
  fetch('https://backend-bingo.onrender.com/submit-form', { // Change this to your Render backend URL
      method: 'POST',
      body: formData
  })
...
// Timer functionality
let timeLeft = 60 * 60; // Set the timer duration to 60 minutes in seconds
